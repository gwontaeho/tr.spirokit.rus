        ...
        .map((_, i) => {
          ...
          return (
            ...
            >
                ...
                {!!measurementId && (
                  <>
                    ...
                    <input
                      ...
                      style={{
                        accentColor:
                          COLORS_GRAPH[type === "pre" ? i : preCount + i],
                      }}
                      onChange={(e) => {
                        setSubject((prev) => {
                    ...
                    />
                    {best && (
                      <span className="material-symbols-outlined absolute top-0 left-0 text-primary">
                        star
                      </span>
                    )}
                  </>
              ...
              {/* <p>{d}</p> */}
              {titles.map((v) => (
                <p key={`meas-fvc-${v.title}`}>
                  {results?.find(({ title }) => title === v.title)?.meas}
                </p>
              ))}
            </li>
          );
