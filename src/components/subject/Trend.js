...
export const Trend = ({ type, data }) => {
  ...
  const maxY = flat.reduce(
    (prev, curr) => (Number(prev.y) > Number(curr.y) ? prev : curr),
    { y: 0 }
  )["y"];
  const tickValues = new Array(
    Math.ceil(Math.ceil(maxY) / Math.ceil(maxY / 6)) + 1 || 0
  )
    .fill(null)
    .map((_, i) => Math.ceil(maxY / 6) * i);
      ...
      <div className="flex-1 relative">
        <ResponsiveLine
          ...
          yScale={{
            type: "linear",
            min: 0,
            max: isFev1per ? 100 : tickValues[tickValues.length - 1],
          }}
          axisBottom={{
            tickSize: 5,
            tickPadding: 5,
        ...
        />
        {isFev1per && (
          <div className="absolute top-[calc((100%-50px)*(3/10)+20px)] left-[50px] w-[calc(100%-70px)] h-[1px] bg-primary" />
        )}
      </div>
