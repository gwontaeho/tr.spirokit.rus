    ...
    <section className="card">
      ...
      {options.map(({ label, isNull, isLoading, hasReport }, i) => {
        return (
          <div
            key={`type-${label}`}
            className="flex text-sm p-4 h-16 items-center"
          >
            <label className="flex-1 flex space-x-4">
              ...
              {!isLoading && !isNull && (
                <input
                  ...
                  checked={type === i}
                  onChange={() =>
                    setSubject((prev) => ({ ...prev, type: i, tab: 0 }))
                  }
                />
              )}
            </label>
            {hasReport &&
              ...
              (isNull ? (
                ...
                <div className="flex space-x-2">
                  <button
                    className="input w-fit h-8"
                    onClick={() => handleClickJpg(i)}
                  >
                    JPG
                  </button>
                  <button
                    className="input w-fit h-8"
                    onClick={() => handleClickPdf(i)}
                  >
                    PDF
                  </button>
                </div>
