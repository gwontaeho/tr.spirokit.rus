...
const Modal = ({ setOpen, date_from, date_to }) => {
  ...
  return (
    ...
    >
      <div
        className="card flex flex-col w-[400px] p-4 space-y-8"
        onClick={(e) => e.stopPropagation()}
      >
        <Image priority src="/logo.svg" alt="logo" width={120} height={20} />
        <div className="flex space-x-4">
          <div className="flex flex-1 flex-col space-y-2">
            <label className="text-sm text-gray-400">{t("PJ.S_DATE")}</label>
            <input
              type="date"
          ...
          <div className="flex flex-1 flex-col space-y-2">
            <label className="text-sm text-gray-400">{t("PJ.E_DATE")}</label>
            <input
              type="date"
              className="input w-full"
              value={to}
  ...
  return (
    <>
      <section className="card">
        <div className="p-4 font-medium">{t("PJ.MEA_HIS")}</div>
      ...
      {open &&
        createPortal(
          <Modal setOpen={setOpen} date_from={date_from} date_to={date_to} />,
          document.body
        )}
    </>
  );
