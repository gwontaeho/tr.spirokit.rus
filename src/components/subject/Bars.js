...
export const Bars = ({ trials }) => {
  ...
  const results =
    trials.find(({ measurementId }) => measurementId === pre_r)?.results || [];
  const post =
    trials.find(({ measurementId }) => measurementId === post_r)?.results || [];
          ...
          <div key={`bar-${title}`} className="card flex flex-col">
            ...
            <div className="px-4 h-24 flex items-center justify-center">
              <div className="relative border rounded-3xl h-8 w-5/6 bg-[#FFF5F5] border-[#FF9191]">
                {!isNaN(posLower) && (
                  <div
                    className="absolute flex items-center justify-center h-full w-[1px] bg-[#FF9191]"
                    style={{ left: `${posLower}%` }}
                  />
                )}
                {!isNaN(posUpper) && (
