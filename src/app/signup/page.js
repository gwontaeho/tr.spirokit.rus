  ...
  return (
    <div className="flex">
      ...
      <div className="flex flex-col space-y-2">
        ...
        {!!message && (
          <span className="w-96 text-xs text-gray-400 peer-aria-[invalid=true]:text-red-600">
            {message}
          </span>
        )}
      </div>
...
export default function Signup() {
  ...
  const checkidMutation = useMutation({
    ...
    onSuccess: ({ response }) => {
      if (response)
        setError("loginId", { type: "d", message: t("signup.m.v_2") });
  ...
  const countries = (countriesQuery.data?.response || []).map(
    ({ name, alpha2 }) => ({ label: name, value: alpha2 })
  );
