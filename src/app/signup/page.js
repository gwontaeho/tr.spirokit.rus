    ...
    return (
        <div className="flex">
            ...
            <div className="flex flex-col space-y-2">
                ...
                {!!message && (
                    <span className="w-96 text-xs text-gray-400 peer-aria-[invalid=true]:text-red-600">{message}</span>
                )}
            </div>
        </div>
    );
...
export default function Signup() {
    ...
    const checkidMutation = useMutation({
        ...
        onSuccess: ({ response }) => {
            if (response) setError("loginId", { type: "d", message: t("PJ.M_5") });
    ...
    const clinicsQuery = useQuery({
        queryKey: ["clinics", alpha2],
        queryFn: () => getClinics(alpha2),
        enabled: !!alpha2,
    });
    const rolesQuery = useQuery({
        queryKey: ["roles", clinicId],
        queryFn: () => getRoles(clinicId),
        enabled: !!clinicId,
    });

    const countries = (countriesQuery.data?.response || []).map(({ name, alpha2 }) => ({ label: name, value: alpha2 }));
    const clinics = (clinicsQuery.data?.response || []).map(({ name, id }) => ({
        label: name,
        value: id,
    }));
    const roles = (rolesQuery.data?.response || []).map(({ name, id }) => ({
        label: name,
        value: id,
    }));
                ...
                <div className="card p-8 flex flex-col space-y-8">
                    ...
                    <form className="flex flex-col space-y-8" onSubmit={handleSubmit(onSubmit)}>
                        ...
                        <Input
                            ...
                            label={t("PJ.ID")}
                            message={errors.loginId?.message || t("PJ.M_4")}
                        ...
                        />
                        <button className="input w-full">{t("PJ.SGN_UP")}</button>
                    </form>
                </div>
            </main>
